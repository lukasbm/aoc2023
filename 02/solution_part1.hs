...
main = do
  ...
  let validGames = filter evaluteGame gamesInputs
  print validGames
  print (sum (map _id validGames)) -- sum valid ids
...
evaluteGame :: Game -> Bool
evaluteGame g = all evaluateDraw (draws g)

evaluateDraw :: Draw -> Bool
evaluateDraw d = red d <= Just 12 && green d <= Just 13 && blue d <= Just 14
